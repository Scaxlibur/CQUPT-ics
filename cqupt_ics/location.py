def get_location(loc):
	from re import search
	try:
		room = search("[0-9]{4}", loc).group()
	except AttributeError:
		room = "6666" # 不存在四位数以上的数字教室匹配
	if "YF" in loc: 
		custom_geo = """LOCATION:重庆邮电大学-逸夫科技楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 -逸夫科技楼\\\\n:geo:29.535617,106.607390"""
	elif "SL" in loc: 
		custom_geo = """LOCATION:重庆邮电大学数理学院\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 数理学院\\\\n:geo:29.530599,106.605454"""
	elif "综合实验" in loc or "实验实训室" in loc: 
		custom_geo = """LOCATION:重庆邮电大学综合实验大楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 综合实验大楼\\\\n:geo:29.524289,106.605595"""
	elif "风华" in loc or loc == "运动场1": 
		custom_geo = """LOCATION:风华运动场\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=\r
 重庆邮电大学风华运动场\\\\n:geo:29.532757,106.607510"""
	elif "太极" in loc:
		custom_geo = """LOCATION:重庆邮电大学-太极体育场\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 -太极体育场\\\\n:geo:29.532940,106.609072"""
	elif "乒乓球" in loc:
		custom_geo = """LOCATION:风雨操场(乒乓球馆)\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=风雨操场(乒乓球馆)\\\\n\r
 :geo:29.534230,106.608516"""
	elif "篮球" in loc or "排球" in loc:
		custom_geo = """LOCATION:重庆邮电大学篮球排球馆\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学篮球排球馆\\\\n\r
 :geo:29.534025,106.609148"""
	elif "仙桃A08" in loc:
		custom_geo = """LOCATION:重庆仙桃数据谷A08\\n中国重庆市渝北区金山大道仙桃国际大数据谷体验中心\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆仙桃数据谷A08\\\\n\r
 中国重庆市渝北区金山大道仙桃国际大数据谷体验中心:geo:29.739791,106.55661"""
	elif "仙桃运动场" in loc:
		custom_geo = """LOCATION:仙桃体育公园\\n中国重庆市渝北区金山大道仙桃国际大数据谷体验中心\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=仙桃体育公园\\\\n\r
 中国重庆市渝北区仙桃街道数据谷东路仙桃国际数据谷:geo:29.745789,106.55749""" 
	elif room[0] == "1":
		custom_geo = """LOCATION:重庆邮电大学-光电工程学院\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 -光电工程学院\\\\n:geo:29.531478,106.605921"""
	elif room[0] == "2": 
		custom_geo = """LOCATION:重庆邮电大学二教学楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 二教学楼\\\\n:geo:29.532703,106.606747"""
	elif room[0] == "3": 
		custom_geo = """LOCATION:重庆邮电大学第三教学楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 第三教学楼\\\\n:geo:29.535119,106.609114"""
	elif room[0] == "4": 
		custom_geo = """LOCATION:重庆邮电大学第四教学楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 第四教学楼\\\\n:geo:29.536107,106.608759"""
	elif room[0] == "5": 
		custom_geo = """LOCATION:重庆邮电大学-国际学院\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 -国际学院\\\\n:geo:29.536131,106.610090"""
	elif room[0] == "8": 
		custom_geo = """LOCATION:重庆邮电大学八教学楼A栋\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 八教学楼A栋\\\\n:geo:29.535322,106.611020"""
	elif room[0] == "9": 
		custom_geo = """LOCATION:重庆邮电大学九教学楼\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=重庆邮电大学\r
 九教学楼\\\\n:geo:29.525881,106.606184"""
	else: #Fallback
		custom_geo = """LOCATION:重庆邮电大学\\n\r
X-APPLE-STRUCTURED-LOCATION;VALUE=URI;X-TITLE=\r
 重庆邮电大学\\\\n:geo:29.530807,106.607617"""
	custom_geo = f'\r\n{custom_geo}\r\nGEO:{custom_geo.split("geo:")[1].replace(",", ";")}'
	return custom_geo